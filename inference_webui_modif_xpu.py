...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    print("enter change_sovits_weights")
    ...
    #)
    print("change_sovits_weights done")
...
try:
    next(change_sovits_weights("SoVITS_weights_v3\F21HuLien_e3_s87_l32.pth"))
except:
    pass

def change_gpt_weights(gpt_path):
    print("enter change_gpt_weights")
...
change_gpt_weights("GPT_weights_v3\F21HuLien-e50.ckpt")
...
def get_tts_wav(
    ...
    print("enter")
    global cache,threads,waittd0,waittd1,waittd2,waittd3
    waittd0 = waittd1 = waittd2 = waittd3 = False
    threads = []
    if ref_wav_path:
        pass
    else:
    ...
    for i_text, text in enumerate(texts):
        print(f"Current threads: {len(threads)}")
        print(f"Text length: {len(texts)}")
        textcounter+=1
        if check_thread() and textcounter <= len(texts):
            print("enter thread")
            counter = len(threads)
            print(counter)
            if counter == 0:
                print("enter td0")
                td0 = threading.Thread(target=get_soundtd0, args=(
                    i_text,
                    text,
                    text_language,
                    ref_free,
                ...
                print("enter td1")
                td1 = threading.Thread(target=get_soundtd1, args=(
                    i_text,
                    text,
                    text_language,
                    ref_free,
                ...
                print("enter td2")
                td2 = threading.Thread(target=get_soundtd2, args=(
                    i_text,
                    text,
                    text_language,
                    ref_free,
                ...
                print("enter td3")
                td3 = threading.Thread(target=get_soundtd3, args=(
                    i_text,
                    text,
                    text_language,
                    ref_free,
                ...
                waittd0 = waittd1 = waittd2 = waittd3 = False
                threads = []
                print(f"threads reset:{len(threads)}")
                print("enter td0")
                td0 = threading.Thread(target=get_soundtd0, args=(
                    i_text,
                    text,
    ...
    sound = (audio_opt * 32767).astype(np.int16)
...
#SoVITS_weight_root = ["SoVITS_weights_v3"]
#GPT_weight_root = ["GPT_weights_v3"]
#for path in SoVITS_weight_root + GPT_weight_root:
    #os.makedirs(path, exist_ok=True)


def get_weights_names(GPT_weight_root, SoVITS_weight_root):
    SoVITS_names = ["SoVITS_weights_v3\F21HuLien_e3_s87_l32.pth"]
    GPT_names = ["GPT_weights_v3\F21HuLien-e50.ckpt"]
...
def generateSound(genContent):
    print("run gen")
    #change_gpt_weights("GPT_weights_v3\F21HuLien-e50.ckpt")
    #change_sovits_weights("SoVITS_weights_v3\F21HuLien_e3_s87_l32.pth")
    inp_ref = "input_wav\\HuLien_wav.wav"
    prompt_text = i18n("我們不能夠叫阿 敵人把我們圍起來")
    prompt_language = i18n("中文")
    text = i18n(genContent)
    ...
    inp_refs = None
    sample_steps = 32
    if_sr_Checkbox = False
    pause_second_slider = 0.3
    print('enter tts')
    get_tts_wav(
        inp_ref, #參考音訊路徑
        prompt_text, #參考音訊文本
        prompt_language, #參考音訊語種
        text, #合成文本
        text_language, #合成語種
        how_to_cut,
        top_k,
...
def generateSoundFamily(genContent):
    print("run gen")
    #change_gpt_weights("GPT_weights_v3\F21HuLien-e50.ckpt")
    #change_sovits_weights("SoVITS_weights_v3\F21HuLien_e3_s87_l32.pth")
    inp_ref = "input_wav\\family_wav.wav"
    prompt_text = i18n("北伐、剿匪、抗日")
    prompt_language = i18n("中文")
    text = i18n(genContent)
    text_language = i18n("中文")
    how_to_cut = i18n("按标点符号切")
...
def selectModule():
    print("請選擇以下功能")
    print("\t 1.使用家屬音檔作為參考音檔")
    print("\t 2.使用胡璉音檔作為參考音檔")
    print("\t 3.離開")
...
def start():
    while(1):
        ...
        select = input("請輸入號碼:")
        if select == '1':
            genContent = input("請輸入要生成的文字:")
            generateSoundFamily(genContent)
        elif select == '2':
            genContent = input("請輸入要生成的文字:")
            generateSound(genContent)
        elif select == '3':
            break
